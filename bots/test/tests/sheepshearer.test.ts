...
const DEFAULT_TIMEOUT_TICKS = 20000;
const { timedOut, ticksRun } = await runTickLoop({
    ...
    getLabel: () => 'sheepshearer',
    getHp: () => ({ current: api.player.levels[3]!, max: api.player.baseLevels[3]! }),
    getError: () => scriptError?.message ?? null,
    getQuestVarp: () => api.getQuestProgress(SHEEP_SHEARER_VARP),
    getSkills: () => ({ Attack: api.getSkill('Attack').level, Crafting: api.getSkill('Crafting').level, Hitpoints: api.getSkill('Hitpoints').level }),
    afkThreshold: 500
});
...
if (timedOut || !scriptDone) {
    ...
    console.error(`TIMEOUT after ${durationSeconds}s (${ticksRun} ticks) - pos=(${pos.x},${pos.z},${pos.level}) quest_varp=${questVarp} crafting_xp=${crafting.exp} inventory=[${inv}]`);
    emitFailureJson({
        error: 'TIMEOUT',
        x: pos.x,
        z: pos.z,
        level: pos.level,
        ticksRun,
        lastState: 'sheepshearer',
        questVarp,
        skills: { Crafting: crafting.level }
    });
