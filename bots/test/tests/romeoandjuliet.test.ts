...
const { romeoAndJuliet } = await import('../../scripts/romeo-and-juliet.ts');

// Romeo & Juliet varp ID (from content/pack/varp.pack: 144=rjquest)
const RJQUEST_VARP = 144;
const STAGE_COMPLETE = 100;

console.log('Starting world for Romeo & Juliet test...');
...
const api = BotManager.spawnBot('romeojulietbot1', async (bot) => {
    try {
...
// This quest is dialog-heavy with lots of walking between Varrock NPCs.
// Should complete in a few thousand ticks.
...
if (!scriptDone) {
    ...
    const questVarp = api.getQuestProgress(RJQUEST_VARP);
...
if (scriptError) {
    ...
    const logPath = path.resolve(import.meta.dir, '..', '..', 'logs', 'romeojulietbot1.log');
...
// Success!
console.log('');
console.log('=== Romeo & Juliet Test Results ===');
