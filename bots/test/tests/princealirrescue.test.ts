...
const { princeAliRescue } = await import('../../scripts/prince-ali-rescue.ts');

// Prince Ali Rescue varp ID (from content/pack/varp.pack: 273=princequest)
const PRINCEQUEST_VARP = 273;
const STAGE_COMPLETE = 110;

console.log('Starting world for Prince Ali Rescue test...');
...
const api = BotManager.spawnBot('princebot5', async (bot) => {
    try {
...
// This quest involves long walks, pickpocketing, shopping, mining, smelting,
// dialog sequences, and multiple NPC interactions — very generous timeout.
...
if (!scriptDone) {
    ...
    const questVarp = api.getQuestProgress(PRINCEQUEST_VARP);
...
if (scriptError) {
    ...
    const logPath = path.resolve(import.meta.dir, '..', '..', 'logs', 'princebot5.log');
...
// Success!
console.log('');
console.log('=== Prince Ali Rescue Test Results ===');
