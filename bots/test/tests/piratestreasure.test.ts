...
const { piratesTreasure } = await import('../../scripts/pirates-treasure.ts');

// Pirate's Treasure varp ID (from content/pack/varp.pack: 71=hunt)
const HUNT_VARP = 71;
const STAGE_COMPLETE = 4;

console.log('Starting world for Pirate\'s Treasure test...');
...
// This is a complex multi-location quest with ship travel — very generous timeout.
...
if (!scriptDone) {
    ...
    const questVarp = api.getQuestProgress(HUNT_VARP);
...
if (scriptError) {
    ...
    const logPath = path.resolve(import.meta.dir, '..', '..', 'logs', 'piratebot1.log');
...
// Verify rewards
const goldRing = api.findItem('Gold ring');
const emerald = api.findItem('Emerald');

if (!goldRing) {
    console.error('MISSING REWARD: No gold ring in inventory');
}
if (!emerald) {
    console.error('MISSING REWARD: No emerald in inventory');
}

// Success!
console.log('');
console.log('=== Pirate\'s Treasure Test Results ===');
console.log(`  Quest varp: ${questVarp} (complete)`);
console.log(`  Gold ring: ${goldRing ? 'yes' : 'no'}`);
console.log(`  Emerald: ${emerald ? 'yes' : 'no'}`);
console.log(`  Duration: ${durationSeconds}s`);
console.log('');

const resultLine = `[RESULT] status=PASS duration=${durationSeconds}s quest_varp=${questVarp} gold_ring=${goldRing ? 'yes' : 'no'} emerald=${emerald ? 'yes' : 'no'}`;
console.log(resultLine);
