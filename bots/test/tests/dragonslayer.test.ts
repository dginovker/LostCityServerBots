...
const { dragonSlayer } = await import('../../scripts/dragon-slayer.ts');

// Dragon Slayer varp ID (from content/pack/varp.pack: 176=dragonquest)
const DRAGON_SLAYER_VARP = 176;
const STAGE_COMPLETE = 10;

console.log('Starting world for Dragon Slayer test...');
...
const api = BotManager.spawnBot('dragonslayerbot1', async (bot) => {
    try {
        // Wait ticks for the bot to be logged in by processLogins
        await bot.waitForTick();
        await bot.waitForTick();

        await dragonSlayer(bot);
        scriptDone = true;
    } catch (err) {
...
// Dragon Slayer is a long quest: requires 32 QP (many prerequisite quests),
// Melzar's Maze combat, ship repair, sailing, and Elvarg fight.
const DEFAULT_TIMEOUT_TICKS = 150000;
...
if (timedOut || !scriptDone) {
    ...
    const questVarp = api.getQuestProgress(DRAGON_SLAYER_VARP);
...
if (scriptError) {
    ...
    const logPath = path.resolve(import.meta.dir, '..', '..', 'logs', 'dragonslayerbot1.log');
...
// Success!
console.log('');
console.log('=== Dragon Slayer Test Results ===');
